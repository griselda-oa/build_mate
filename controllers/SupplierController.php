...
class SupplierController extends Controller
{
    ...
    public function dashboard(): void
    {
        ...
        if (empty($supplier['business_name']) || $supplier['kyc_status'] === NULL) {
            $this->setFlash('info', 'Please complete your supplier application form first.');
            $this->redirect('/supplier/kyc');
            return;
        }
        ...
        $products = $supplierModel->getRecentProducts($supplierId, 5);
        $orders = $supplierModel->getRecentOrders($supplierId, 5);
        $stats = $supplierModel->getSupplierStats($supplierId);
        
        // Check premium status
        $isPremium = $supplierModel->isPremium($supplierId);
        ...
        try {
            ...
            $advertisements = $adModel->getBySupplier($supplierId);
    ...
    public function submitKyc(): void
    {
        ...
        try {
            // Update supplier info - map to actual column names in your database
            $fullAddress = $address . ', ' . $city . ', ' . $region;
            
            $updateData = [
                'business_name' => $businessName,
                'business_address' => $fullAddress,  // Your table uses business_address
                'kyc_status' => 'pending'  // Only set to pending after form is submitted
            ];
            
            // Add phone if provided
            if (!empty($_POST['phone'])) {
                $updateData['phone'] = Validator::sanitize($_POST['phone']);
            }
    ...
    public function orders(): void
    {
        ...
        $allOrders = $supplier ? $orderModel->getBySupplier($supplierId) : [];
        ...
        foreach ($orders as &$order) {
            ...
            // Get delivery status
            $delivery = $deliveryModel->findByOrderId((int)$order['id']);
     ...
     */
    public function markReadyForPickup($orderId): void
    {
        $orderId = (int)$orderId;
        $user = $this->user();
        $supplierModel = new Supplier();
    ...
    public function initiateUpgrade(): void
    {
        ...
        $supplierId = (int)$supplier['id'];
        if ($supplierModel->isPremium($supplierId)) {
        ...
        $paymentReference = 'PREMIUM-' . $supplierId . '-' . time() . '-' . bin2hex(random_bytes(4));
    ...
    public function premiumStatus(): void
    {
        ...
        $isPremium = $supplierModel->isPremium($supplierId);
        $subscriptionModel = new PremiumSubscription();
        $subscriptions = $subscriptionModel->getBySupplier($supplierId);
