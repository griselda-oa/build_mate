...
class DB
{
    ...
    public static function getInstance(): PDO
    {
        if (self::$instance === null) {
            ...
            // For macOS XAMPP, try socket first (most reliable), then TCP
            $host = $db['host'];
            
            // Build DSN - prefer unix socket for localhost connections
            if ($host === 'localhost' || $host === '127.0.0.1') {
                // Try to find XAMPP socket file
                $socketPaths = [
                ...
                // Use socket if found, otherwise fall back to TCP
                if ($socketPath) {
                    $dsn = sprintf(
