...
class Order extends Model
{
    ...
    public function createFromCart(int $buyerId, array $cartItems, array $address, Product $productModel): int
    {
        ...
        try {
            // Get supplier ID from first product (assuming single supplier per order)
            $firstProduct = $productModel->find($cartItems[0]['product_id']);
            $supplierId = $firstProduct['supplier_id'] ?? 0;
